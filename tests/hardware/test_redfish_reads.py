...
The broad sweep. It does not assert what a BMC answers, because that varies
legitimately by model and vendor. It asserts how it answers: either a result,
or a refusal that says why. What must never happen is a crash inside
confluent, which reaches a user as "Unexpected Error" with a traceback in the
server log and tells them nothing.
...
async def test_read_answers_or_refuses_with_a_reason(redfish_command,
                                                     operation):
    method = getattr(redfish_command, operation, None)
    if method is None:
...
async def test_client_implements_what_confluent_calls(redfish_command):
    ...
    missing = [name for name in universal
               if getattr(redfish_command, name, None) is None]
    assert missing == [], 'client is missing: {0}'.format(', '.join(missing))


async def test_identify_reports_a_state_a_caller_can_act_on(redfish_command):
    """Either one of the documented states, or a stated refusal."""
    try:
        state = await redfish_command.get_identify()
    except BUG_SHAPES as caught:
