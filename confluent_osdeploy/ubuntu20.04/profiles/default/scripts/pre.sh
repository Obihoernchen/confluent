...
for pubkey in /etc/ssh/ssh_host*key.pub; do
    ...
    keyfile=${pubkey%.pub}
    curl -f -X POST -H "CONFLUENT_NODENAME: $nodename" -H "CONFLUENT_APIKEY: $apikey" -d @$pubkey https://$confluent_whost/confluent-api/self/sshcert > $certfile
...
curl -f https://$confluent_whost/confluent-public/os/$confluent_profile/scripts/functions > /etc/confluent/functions
. /etc/confluent/functions
run_remote_parts pre.d
curl -f -X POST -H "CONFLUENT_NODENAME: $nodename" -H "CONFLUENT_APIKEY: $apikey" https://$confluent_whost/confluent-api/self/nodelist > /tmp/allnodes
if [ ! -e /tmp/installdisk ]; then
    curl -f https://$confluent_whost/confluent-public/os/$confluent_profile/scripts/getinstalldisk > /custom-installation/getinstalldisk
